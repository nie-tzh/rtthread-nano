...
IRQ 27由8个通道共享，处理流程应为：

```text
进入IRQ 27
    -> 遍历或按活动掩码检查各通道TxIntStatus
    -> 对每个pending通道读取TxEOI
    -> Tick通道pending时只调用一次rt_tick_increase()
    -> 处理其他已注册通道，不能误清或遗漏
    -> 确认共享中断线已撤销
    -> 返回
```
...
- 选择哪个空闲通道作为系统Tick。
- 该通道是否与延时、超时、帧同步或心跳功能冲突。
- 实际APB频率及其是否会动态变化。
- `TxLoadCount`的计算、取整和边界。
- 共享IRQ中的扫描顺序和最坏处理时间。
- 长时间Tick漂移和丢Tick行为。

## 第五部分 验证目标

架构文档不展开测试实现；具体测试方法、构建命令、预期日志和失败分析见独立验证文档。
...
| 后续系统验证 | DW Timer、上下文切换、RT-Thread调度和长期运行 |
