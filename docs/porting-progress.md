...
| 5. 建立异常与CLIC中断机制 | 进行中 | 已完成同步异常入口与现场验证；待完成CLIC初始化和软件中断 |
...
详细构建、运行、预期日志和失败分析见[《E902异常与现场验证》](e902-interrupt-validation.md)。

### 5.3 T22 CLIC初始化与软件中断验证（待开始）

建议提交信息：

```text
Porting: 添加T22 CLIC初始化与软件中断验证
```

验收条件：

- 初始化CLIC全局配置和已实现的中断源。
- 建立64字节对齐的`mtvt`硬件向量表。
- 使用Machine Software Interrupt，IRQ 3，完成一次可控中断测试。
- 中断处理后能够清除请求并正确返回主程序。
- 全局中断只在入口、向量和处理函数全部就绪后打开。
