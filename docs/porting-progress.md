...
| 6. 实现DW Timer并验证周期中断 | 进行中 | 建立T22 DW Timer周期时基并完成上板验证 |
...
具体构建命令、实测日志和失败码见[《E902异常与CLIC验证》](e902-interrupt-validation.md)。第5阶段已经完成，第6阶段的DW Timer必要代码已经加入，下一步通过独立应用完成周期中断上板验证。
...
- `demo`、异常测试和CLIC测试应用均可无警告编译、链接。
- 使用更严格的额外告警选项检查新增C文件。
- 强制保留全部Timer公开接口完成链接，确认没有被链接垃圾回收掩盖的未定义符号。
- ELF保持RV32E、RVC和ILP32E属性，入口及内存布局不变。

第6阶段尚未完成。后续验证提交需要独立测试应用和验证文档，至少确认：

- TIMER1以目标频率持续产生IRQ 27。
- 每次中断只增加一次回调计数，EOI后中断线能正确撤销。
- 启停和再次启动行为正确，不影响其他Timer通道。
- 实测周期、累计漂移、丢Tick和长时间运行行为满足要求。
